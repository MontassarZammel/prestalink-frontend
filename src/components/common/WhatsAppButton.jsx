import { useState, useEffect } from 'react';
import { motion, AnimatePresence } from 'framer-motion';
import { X } from 'lucide-react';
import api from '../../services/api';

const WaIcon = () => (
  <svg viewBox="0 0 24 24" fill="currentColor" width="22" height="22">
    <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z"/>
  </svg>
);

export default function WhatsAppButton() {
  const [phone, setPhone]   = useState(null);
  const [open, setOpen]     = useState(false);
  const [dismissed, setDismissed] = useState(false);

  useEffect(() => {
    api.get('/settings').then(r => {
      const num = r.data.data?.whatsapp_number;
      if (num) setPhone(num.replace(/\D/g, ''));
    }).catch(() => {});
  }, []);

  if (!phone || dismissed) return null;

  const href = `https://wa.me/${phone}?text=${encodeURIComponent('Bonjour, j\'ai une question concernant MyWedding. Pouvez-vous m\'aider ?')}`;

  return (
    <div className="fixed bottom-24 right-6 z-[499] flex flex-col items-end gap-3">

      {/* Popup card */}
      <AnimatePresence>
        {open && (
          <motion.div
            initial={{ opacity: 0, y: 16, scale: 0.95 }}
            animate={{ opacity: 1, y: 0,  scale: 1 }}
            exit={{  opacity: 0, y: 16, scale: 0.95 }}
            transition={{ type: 'spring', damping: 20, stiffness: 300 }}
            className="w-72 rounded-2xl overflow-hidden shadow-2xl"
            style={{ background: 'var(--s2)', border: '1px solid var(--border-2)' }}>

            {/* Header */}
            <div className="px-4 py-4 flex items-center justify-between" style={{ background: '#075E54' }}>
              <div className="flex items-center gap-3">
                <div className="w-10 h-10 rounded-full flex items-center justify-center text-white font-bold text-lg flex-shrink-0"
                  style={{ background: '#128C7E' }}>
                  MW
                </div>
                <div>
                  <p className="text-white font-semibold text-sm leading-tight">Support MyWedding</p>
                  <div className="flex items-center gap-1.5 mt-0.5">
                    <span className="w-2 h-2 rounded-full bg-green-400 inline-block" />
                    <span className="text-green-300 text-xs">En ligne</span>
                  </div>
                </div>
              </div>
              <button onClick={() => setOpen(false)}
                className="w-7 h-7 rounded-full flex items-center justify-center transition-colors hover:bg-white/10">
                <X size={14} className="text-white" />
              </button>
            </div>

            {/* Message bubble */}
            <div className="px-4 py-4" style={{ background: '#ECE5DD' }}>
              <div className="bg-white rounded-xl rounded-tl-sm px-3 py-2.5 shadow-sm max-w-[85%]">
                <p className="text-sm leading-relaxed" style={{ color: '#111' }}>
                  Bonjour 👋<br />
                  Une question sur votre mariage ? Je suis là pour vous aider !
                </p>
                <p className="text-right text-[10px] mt-1" style={{ color: '#aaa' }}>
                  {new Date().toLocaleTimeString('fr-TN', { hour: '2-digit', minute: '2-digit' })}
                </p>
              </div>
            </div>

            {/* CTA */}
            <div className="px-4 py-3">
              <a href={href} target="_blank" rel="noopener noreferrer"
                className="flex items-center justify-center gap-2 w-full py-2.5 rounded-xl font-semibold text-sm text-white transition-opacity hover:opacity-90"
                style={{ background: '#25D366' }}>
                <WaIcon />
                Ouvrir WhatsApp
              </a>
            </div>
          </motion.div>
        )}
      </AnimatePresence>

      {/* FAB button */}
      <div className="relative">
        {!open && (
          <motion.span
            initial={{ scale: 0 }} animate={{ scale: 1 }} transition={{ delay: 0.3 }}
            className="absolute -top-1 -right-1 w-3 h-3 rounded-full z-10"
            style={{ background: '#25D366', border: '2px solid white' }}>
            <span className="absolute inset-0 rounded-full animate-ping" style={{ background: '#25D366', opacity: 0.6 }} />
          </motion.span>
        )}
        <motion.button
          onClick={() => setOpen(o => !o)}
          whileHover={{ scale: 1.08 }} whileTap={{ scale: 0.93 }}
          className="w-14 h-14 rounded-full flex items-center justify-center text-white shadow-xl"
          style={{ background: '#25D366' }}>
          <AnimatePresence mode="wait">
            {open
              ? <motion.span key="x"  initial={{ rotate: -90, opacity: 0 }} animate={{ rotate: 0, opacity: 1 }} exit={{ rotate: 90, opacity: 0 }} transition={{ duration: 0.15 }}><X size={22} /></motion.span>
              : <motion.span key="wa" initial={{ rotate:  90, opacity: 0 }} animate={{ rotate: 0, opacity: 1 }} exit={{ rotate: -90, opacity: 0 }} transition={{ duration: 0.15 }}><WaIcon /></motion.span>
            }
          </AnimatePresence>
        </motion.button>
      </div>
    </div>
  );
}
